...
export const ScalpingConfigSchema = z.object({
  ...
  symbols: z.array(Symbol).min(1).default(["EURUSD", "GBPUSD", "USDJPY", "AUDUSD", "USDCAD"]),
  ...
  aiDecisionTtlSeconds: z.number().int().positive().default(60),
});
...
export const ScalpingRiskSchema = z.object({
  ...
  maxOpenTradesTotal: z.number().int().min(1).default(5),
  // Multi-entry is supported up to the account-wide open-trade ceiling. The
  // execution layer additionally requires an MT5 hedging account for values >1.
  ...
  targetProfitMoney: z.number().positive().default(0.2),
  maxLossMoney: z.number().positive().default(0.05),
  ...
  takeProfitPoints: z.number().positive().nullable().default(null),
  stopLossPoints: z.number().positive().nullable().default(null),
  maxLotSize: z.number().positive().default(0.01),
  ...
  riskPerTradePercent: z.number().positive().default(0.5),
  maxTotalRiskExposurePercent: z.number().positive().default(5),
  ...
  profitTargetMoney: z.number().positive().nullable().default(null),

  reentryAfterWinSeconds: z.number().int().min(1).default(1),
  reentryAfterLossSeconds: z.number().int().min(1).default(60),

  maxTradesPerDay: z.number().int().min(1).default(100),
  maxConsecutiveLosses: z.number().int().min(1).default(3),
  pauseAfterLossStreakMinutes: z.number().int().min(0).default(10),

  dailyLossLimitMoney: z.number().positive().nullable().optional(),
  // Realistically capped: a daily loss cap above 50% of balance is almost
  // certainly a typo and would defeat the protection.
  dailyLossLimitPercent: z.number().positive().nullable().optional().default(1.5),

  maxSharedCurrencyExposure: z.number().int().min(1).default(2),
  maxSpreadPointsBySymbol: z.record(z.string(), z.number().positive()).default({
    EURUSD: 15,
    GBPUSD: 20,
    USDJPY: 18,
    AUDUSD: 20,
  ...
  pauseBeforeNewsMin: z.number().int().min(0).default(15),
  pauseAfterNewsMin: z.number().int().min(0).default(5),
