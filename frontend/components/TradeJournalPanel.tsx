...
export function TradeJournalPanel() {
  ...
  const save = async () => { if (!tradeId) return; await api(`/api/trades/${tradeId}/journal`, { method: "PUT", body: { notes, lessons, tags: tags.split(",").map((tag) => tag.trim()).filter(Boolean), rating: rating ? Number(rating) : null } }); setNotes(""); setTags(""); setLessons(""); setRating(""); await load(); };
